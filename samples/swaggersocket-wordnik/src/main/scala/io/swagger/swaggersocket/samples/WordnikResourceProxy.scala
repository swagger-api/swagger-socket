...
import io.swagger.api.client.model.{Definition, ExampleSearchResults}
import reflect.BeanProperty
import io.swagger.swagger.runtime.common.{ApiKeyAuthTokenBasedSecurityHandler, APIInvoker}
...
class WordnikResourceProxy extends RestResourceUtil {
  ...
  def getExamples(
    ...
    val data: ExampleSearchResults = io.swagger.api.client.api.WordAPI.getExamples(word,
        null,
        useCanonical,
        null,
        skip,
  ...
  def getDefinitions(
    ...
    @ApiParam(value = "Return a closed set of XML tags in response", defaultValue = "false", allowableValues = "false,true")@QueryParam("includeTags") includeTags: String) = {
      val data = io.swagger.api.client.api.WordAPI.getDefinitions(
        word,
        limit,
        null,
        null,
  ...
  @GET
  @Path("/{word}/details")
  @ApiOperation(value = "Returns the definitions/examples for a given word", response = classOf[io.swagger.swaggersocket.samples.WordDetails])
  def getWordDetails(
    @ApiParam(value = "Word to return details for", required = true)@PathParam("word") word: String,
    @ApiParam(value = "If true will try to return the correct word root ('cats' -> 'cat'). If false returns exactly what was requested.", allowableValues = "false,true")@QueryParam("useCanonical") useCanonical: String,
    @ApiParam("Results to skip")@QueryParam("skip") skip: String,
    @ApiParam("Maximum number of results to return")@QueryParam("limit") limit: String): Response = {
      val examples = io.swagger.api.client.api.WordAPI.getExamples(word,
        null,
        useCanonical,
        null,
        skip,
        limit)
      val definitions = io.swagger.api.client.api.WordAPI.getDefinitions(
        word,
        limit,
        null,
        "true",
